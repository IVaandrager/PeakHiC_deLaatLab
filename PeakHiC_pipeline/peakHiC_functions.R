...
rdsFldr <- "/delaat/group/geert/peakHiC/ANALYSES/mESC_OCT4_GFP/mm10_lncRNA_peakHiC/rds/"

hicCond <- "mESC_OCT4_GFP"
...
getPartitionPeaks <- function(partID, genomeObj, hicCond="mESC_OCT4_GFP", wSize=41, qWr=1, alphaFDR=0.1,  minDist=30e3 , nReps=NULL) {
	...
	loopsFldr <- paste0("/delaat/group/geert//peakHiC/ANALYSES/mESC_OCT4_GFP/mm10_lncRNA_peakHiC/rds/loops/")
...
getChrPeaks <- function(chr, genomeObj, hicCond="mESC_OCT4_GFP", wSize=41, qWr=1, alphaFDR=0.1,  minDist=30e3 , nReps=NULL, nThreads=8) {
...
getPeakCPeaksWithReps <- function( vpID, vpReads, partVPs, hicCond="mESC_OCT4_GFP", wSize=41, qWr=1, alphaFDR=0.1,  minDist=30e3 ) {
...
rdsFldr <- "/data2/repository/misha/mm10/rds/V4C/mESC_OCT4_GFP/"
...
getLoopCovbyPartition <- function(partID,loops,genomeObj,hicCond="mESC_OCT4_GFP",anchorSize=10e3) {
	...
	if(nrow(partLoops)>0) {
		...
		tracks <- hicTracksByCondition[[hicCond]] #slices
		...
		for(trackID in tracks) {
			...
			PRreads <- getPartitionReads(partID=partID,trackID=trackID,genomeObj=genomeObj)
